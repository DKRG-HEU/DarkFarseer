...
def masked_corr_coefficient(pred, true, mask):
    pred_mean = np.sum(pred * mask) / np.sum(mask)
    true_mean = np.sum(true * mask) / np.sum(mask)
    numerator = np.sum(((pred - pred_mean) * (true - true_mean)) * mask)
    denominator = np.sqrt(np.sum(((pred - pred_mean) ** 2) * mask) * np.sum(((true - true_mean) ** 2) * mask))
